...
def save_campaign(request):
    ...
    action         = request.POST.get('action', 'draft')        # 'draft' | 'send'
    campaign_name  = request.POST.get('campaign_name', '').strip()
    recipient_type = request.POST.get('recipient_type', 'list').strip()  # 'list' | 'segment'
    ...
    if campaign_segment:
        ...
        total_recipients = campaign_list.total_count
...
def create_campaign(request):
    ...
    active_segments   = Segment.objects.filter(user_id=user_id, status='active', deleted_at__isnull=True).order_by('name')
